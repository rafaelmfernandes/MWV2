...
async inicializar() {
    ...
    try {
         ...
         * O NOME DO USUÁRIO É OBTIDO AQUI PARA NÃO
         * DEPENDER DO CAMPO nome DA TABELA perfis.
         */
        await this.carregarUsuario(usuarioId);
...
async carregarUsuario(usuarioId) {
    ...
    try {
        ...
        const {
            ...
            .from(this.CONFIG.tabelas.usuarios)
            .select("*")
            .eq("id", usuarioId)
            .maybeSingle();
...
preencherPerfil() {
    ...
    /* -------------------------------------------------
       ...
       PRIORIDADE:
       1. usuarios.nome
       2. usuarios.nome_completo
       3. usuarios.nome_artistico
       4. perfis.nome
       5. perfis.nome_completo
       6. perfis.nome_artistico
       7. Cantor
       */
...
renderizarPortfolio() {
    ...
    this.renderizarImagens(imagens);

    this.renderizarVideos(videos);

    this.renderizarAudios(audios);
},
