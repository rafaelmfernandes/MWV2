const MusicalWorldMeuPerfilMusico = (() => {
    ...
    function preencherServicos(
        ...
        servicos.forEach(
            (servico) => {
                 ...
                 * IMPORTANTE:
                 *
                 * A estrutura abaixo segue a mesma
                 * organização visual dos cards reais
                 * utilizados em editar-perfil-musico.
                 *
                 * Não existem botões de editar/excluir
                 * porque esta é a página de visualização.
                 */

                card.className =
                    "servico-edit-card perfil-servico-card";
    ...
    function obterPrecoServico(
        ...
        if (
            tipoPreco === "a_partir" ||
            tipoPreco === "a_partir_de" ||
            tipoPreco === "a partir de"
        ) {
            return valor > 0
                ? `A partir de ${formatarMoeda(valor)}`
                : "A partir de";
        }
