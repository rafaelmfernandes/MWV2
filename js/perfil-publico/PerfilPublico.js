...
* Direcionar para a página de apresentação específica
  de cada tipo de perfil.

PÁGINAS DE APRESENTAÇÃO:

Cantor(a)            → apresentar-perfil-cantor.html
Músico(a)            → apresentar-perfil-musico.html
Banda                → apresentar-perfil-banda.html
Dupla musical        → apresentar-perfil-dupla.html
DJ                   → apresentar-perfil-dj.html
Dançarino(a)         → apresentar-perfil-dancarino.html
Grupo de dança       → apresentar-perfil-grupo-danca.html
MC                   → apresentar-perfil-mc.html
Compositor(a)        → apresentar-perfil-compositor.html
Produtor(a) musical  → apresentar-perfil-produtor-musical.html
Contratante          → apresentar-perfil-contratante.html

IMPORTANTE:

* Cada tipo de perfil possui sua própria página.
* Não existe uma página genérica "apresentar-perfil.html".
* O ID do perfil é enviado através da URL:
  ?id=ID_DO_PERFIL
  ========================================================= */
...
pagina: {
    ...
    /*
     * Tipo padrão somente como fallback.
     *
     * A página de apresentação NÃO deve ser definida
     * por este valor quando o perfil já possui seu tipo.
     */
     ...
     * -------------------------------------------------
     *
     * Cada tipo possui sua própria página.
     */
...
if (!pagina) {
    ...
    mostrarToast(
        "A página de apresentação deste perfil ainda não está disponível.",
        "erro"
    );
...
if (!pagina) {
    ...
    mostrarToast(
        `Não existe página de edição configurada para o perfil "${tipo}".`,
        "erro"
    );


    aviso(
        "Página de edição não encontrada para o tipo:",
        tipo
    );
...
/*
 * Primeiro identificamos o tipo REAL do perfil.
 *
 * A prioridade é:
 *
 * 1. perfilArtista.tipo_artista
 * 2. perfilArtista.tipoArtista
 * 3. perfilArtista.tipo
 * 4. perfil.tipo_artista
 * 5. perfil.tipoArtista
 * 6. perfil.tipo
 * 7. usuario.tipo_perfil
 * 8. usuario.tipoPerfil
 *
 * O fallback "Cantor(a)" só é utilizado se nenhum
 * tipo tiver sido encontrado.
 */
...
const pagina =
    CONFIG.pagina.apresentacao?.[
        tipo
    ] || "";


log(
    "Identificação da página de apresentação:",
    {
...
if (!pagina) {
    ...
    aviso(
        "Nenhuma página de apresentação configurada para o tipo:",
        tipo
    );
...
if (!pagina) {
    ...
    aviso(
        "Não foi possível construir o link: página de apresentação não encontrada."
    );
...
/*
 * Mantém a estrutura atual da aplicação.
 *
 * Exemplo:
 *
 * https://seudominio.com/
 * apresentar-perfil-cantor.html?id=123
 *
 * ou:
 *
 * https://seudominio.com/
 * apresentar-perfil-musico.html?id=456
 */
...
log(
    "Link de apresentação construído:",
    url
);
